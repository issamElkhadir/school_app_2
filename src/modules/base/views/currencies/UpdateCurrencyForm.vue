<template>
  <ResourceUpdate resource="currencies"
                  :id="+id"
    ...
    <template #subtitle="{ original }">
      <PlfBreadcrumb>
        ...
        <PlfBreadcrumbEl label="Currencies"
                         :to="{ name: 'admin.currencies.table' }" />
    ...
    <template #actions="{ disabled, saving, save, toggleEdit }">
      <template v-if="disabled">
        ...
        <PlfButton label="New"
                   :to="{ name: 'admin.currencies.create' }"
                   icon="mdi.IconPlus"
                   color="primary" />
      </template>
    ...
    <template #form="props">
      <h3 class="card-title">Update Currency {{ props.original.name }}</h3>

      <CurrencyForm v-model:record="props.record"
