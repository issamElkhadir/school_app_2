<template>
  <ResourceUpdate resource="accounting/cash-registers"
                  :id="id"
                  :title="$t('')">

    <template #subtitle="{ original }">
      <PlfBreadcrumb>
        <PlfBreadcrumbEl label="Accounting"
                         class="text-danger"
    ...
    <template #actions="{ disabled, saving, save, toggleEdit }">
      <template v-if="disabled">
        ...
        <PlfButton label="New"
                   :to="{ name: 'admin.accounting.cash-registers.create' }"
                   icon="mdi.IconPlus"
                   color="primary" />
      </template>
    ...
    <template #form="props">
      <h3 class="card-title">Update {{ props.original.name }}</h3>
