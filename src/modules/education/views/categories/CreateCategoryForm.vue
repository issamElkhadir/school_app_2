<template>
  <ResourceCreate resource="education/categories"
    ...
    <template #form="props">
      <h3 class="card-title">{{ $t('categories.create') }}</h3>

      <CategoryForm v-model="props.record"
...
const onSaved = ({id}) => {
  router.push({name: 'admin.categories.update', params: {id}});
}
