<template>
    <ResourceUpdate resource="education/pre-inscriptions"
                    :id="+id" >
      ...
      <template #subtitle="{ original }">
        <PlfBreadcrumb>
          ...
          <PlfBreadcrumbEl :label="$t('pre-inscriptions.title')"
                           :to="{ name: 'admin.pre-inscriptions.table' }"/>
      ...
      <template #actions="{ disabled, saving, save, toggleEdit }">
        <template v-if="disabled">
          ...
          <PlfButton label="New"
                     :to="{ name: 'admin.pre-inscriptions.create' }"
                     icon="mdi.IconPlus"
                     color="primary"/>
        </template>
      ...
      <template #form="{ record, validation, original, disabled }">
        ...
        <PreInscriptionForm :model-value="record"
  ...
  import PreInscriptionForm from "@education/views/pre-inscriptions/PreInscriptionForm.vue";
