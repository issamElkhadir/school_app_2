<template>
  <ResourceUpdate resource="education/periods"
                  :id="id"
                  :title="$t('')">
    ...
    <template #subtitle="{ original }">
      <PlfBreadcrumb>
        ...
        <PlfBreadcrumbEl label="periods"
                         :to="{ name: 'admin.periods.table' }"/>
    ...
    <template #form="{ record, original, validation, disabled }">
      <h3 class="card-title">Update Period {{ original.name }}</h3>
