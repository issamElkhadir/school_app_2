...
const onFauxIframeClick = ({ el, event, handler, middleware }) => {
    ...
    setTimeout(() => {
        ...
        if (
            ...
            execHandler({ event: event, handler, middleware });
        }
    }, 0);
};
...
const beforeMount = (el, { value }) => {
    ...
    el[HANDLERS_PROPERTY] = events.map((eventName) => ({
        ...
        handler: (event) => onEvent({ el, event: event, handler, middleware }),
        capture,
    }));
    if (detectIframe) {
        const detectIframeEvent = {
            event: "blur",
            srcTarget: window,
            handler: (event) => onFauxIframeClick({ el, event: event, handler, middleware }),
            capture,
        };
