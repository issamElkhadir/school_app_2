<script>
import { computed, defineComponent, ref, h, watch, toRef } from 'vue'
...
const fixedStyles = { padding: 0, border: 0, margin: 0 }
const perfStyles = { willChange: 'height' }

export default defineComponent({
  name: 'PlfExpandTransitionV2',
  inheritAttrs: true,
  props: {
  ...
  setup(props, { slots }) {
    ...
    const style = ref(isExpanded.value ? fixedStyles : collapseStyles.value.collapsed)
    const state = ref(isExpanded.value ? 'expanded' : 'collapsed')
      ...
      if (event.target === collapseRef.value && event.propertyName === 'height') {
        ...
        if (isExpanded.value) {
          if (
            ...
            parseFloat(event.target.style.height)
          ) {
...
function getHeightStyles(height = 0, baseHeight = 0) {
  return {
    '--plf-auto-duration': `${getAutoDuration(height - baseHeight)}ms`,
    height: `${height}px`,
  }
}
